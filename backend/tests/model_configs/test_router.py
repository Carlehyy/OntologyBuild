def test_legacy_model_config_facades_preserve_object_identity():
    from app.models import model_config as legacy_model
    from app.routers import models as legacy_router
    from app.schemas import model_config as legacy_schema
    from app.services import model_config_selector as legacy_selector
    from app.model_configs import models, router, schemas, selector

    assert legacy_router.router is router.router
    assert legacy_model.ModelConfig is models.ModelConfig
    assert legacy_model.ModelCallLog is models.ModelCallLog
    assert legacy_schema.ModelConfigCreate is schemas.ModelConfigCreate
    assert legacy_schema.ModelConfigUpdate is schemas.ModelConfigUpdate
    assert legacy_schema.ModelConfigOut is schemas.ModelConfigOut
    assert legacy_selector.select_llm_model_config is selector.select_llm_model_config
    assert legacy_selector.llm_call_kwargs is selector.llm_call_kwargs


def test_model_config_openapi_contract_is_stable(client):
    schema = client.app.openapi()
    expected = {
        ("get", "/api/v1/models"): "list_models_api_v1_models_get",
        ("post", "/api/v1/models"): "create_model_api_v1_models_post",
        (
            "post",
        ...
        (
            "get",
            "/api/v1/models/{model_id}/calls",
        ): "list_model_calls_api_v1_models__model_id__calls_get",
        (
            "post",
            "/api/v1/models/{model_id}/default",
        ): "set_default_model_api_v1_models__model_id__default_post",
        (
            "post",
            "/api/v1/models/{model_id}/enabled",
        ): "set_model_enabled_api_v1_models__model_id__enabled_post",
        (
            "get",
            "/api/v1/models/{model_id}/stats",
        ): "get_model_stats_api_v1_models__model_id__stats_get",
        (
            "post",
