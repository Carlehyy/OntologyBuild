...
def test_single_row_helpers_keep_create_commit_refresh_order(
    ...
    workflow = workflow_config_service._get_workflow_config(db)
...
def test_workflow_success_keeps_two_phase_create_then_save_order(
    db,
    monkeypatch,
):
    events = []
    ...
    def test_connection(**kwargs):
        events.append(("test", kwargs))
        return SimpleNamespace(ok=True, api_base="http://n8n/api/v1")

    def encrypt_key(value):
        events.append(("encrypt", value))
    ...
    assert response.model_dump() == {
        "ok": True,
        "message": "n8n 连接成功",
        "api_base": "http://n8n/api/v1",
    }
    assert events == [
        (
            "test",
            {
                "api_url": "http://n8n/api/v1",
                "api_key": "secret-key",
                "timeout_seconds": 25,
            },
        ),
    ...
    saved = db.query(WorkflowConfig).filter_by(id="default").one()
    assert saved.enabled is True
    assert saved.api_url == "http://n8n/api/v1"
    assert saved.api_key_encrypted == "encrypted:secret-key"
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda _http: N8nApiError(401, "invalid key"),
            "n8n API 返回错误: invalid key",
        ),
        (
            lambda http: http.ConnectError("offline"),
            "无法连接到 n8n 服务，请检查地址是否正确",
        ),
        (
            lambda http: http.TimeoutException("slow"),
            "连接 n8n 超时，请检查网络或调大超时时间",
        ),
        (
            lambda _http: ValueError("invalid response"),
            "invalid response",
        ),
        (
            lambda _http: RuntimeError("broken"),
            "连接 n8n 失败: broken",
        ),
    ],
)
def test_workflow_connection_error_mapping_never_persists(
    error_factory,
    message,
):
    http = _HttpxModule(lambda _url, **_kwargs: None)
    db = SimpleNamespace(
        commit=lambda: pytest.fail("failed connection must not commit"),
    )

    def fail_connection(**_kwargs):
        raise error_factory(http)
    ...
    response = workflow_config_service.test_workflow_connection(
        ...
        test_connection_fn=fail_connection,
