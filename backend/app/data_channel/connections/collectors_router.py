...
def collect_aihot(ontology_id: str, body: CollectRequest,
    ...
    if body.source_object_type_id:
        for si in db.query(ObjectInstance).filter(
                ObjectInstance.ontology_id == ontology_id,
                ObjectInstance.object_type_id == body.source_object_type_id).all():
    ...
    for item in items:
        ...
        existing = db.query(ObjectInstance).filter(
            ObjectInstance.ontology_id == ontology_id,
            ObjectInstance.object_type_id == ot.id,
            ObjectInstance.external_id == ext).first()
        ...
        if existing:
            ...
            old_props = None
            inst = ObjectInstance(ontology_id=ontology_id, object_type_id=ot.id,
            ...
            if sname:
                ...
                if not sid:
                    sinst = ObjectInstance(
                        ontology_id=ontology_id, object_type_id=body.source_object_type_id,
                    ...
                    record_property_facts(
                        ...
                        source="collector://aihot")
                # 去重连边
                exists_link = db.query(LinkInstance).filter(
                    ...
                    LinkInstance.target_object_id == sid).first()
                if not exists_link:
                    li = LinkInstance(
                        ontology_id=ontology_id, link_type_id=body.source_link_type_id,
