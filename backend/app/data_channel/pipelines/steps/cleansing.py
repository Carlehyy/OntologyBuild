...
# 待规范化为 ISO 8601 的日期模式：(正则, 捕获组顺序 → (年,月,日) 的索引)
_DATE_PATTERNS = [
    (re.compile(r'^(\d{4})[-/](\d{1,2})[-/](\d{1,2})$'), (0, 1, 2)),   # YYYY-M-D
    (re.compile(r'^(\d{1,2})[-/](\d{1,2})[-/](\d{4})$'), (2, 1, 0)),   # D/M/YYYY
    (re.compile(r'^(\d{4})(\d{2})(\d{2})$'), (0, 1, 2)),               # YYYYMMDD
]


def _normalize_date(val: str) -> str:
    """将日期字符串标准化为 YYYY-MM-DD。
    旧实现的格式串把 D/M/Y 拼成 YYYY-DD-MM、YYYYMMDD 丢零填充，产出非法日期。"""
    val = val.strip()
    for pat, (yi, mi, di) in _DATE_PATTERNS:
        m = pat.match(val)
        if m:
            parts = [int(g) for g in m.groups()]
            y, mo, d = parts[yi], parts[mi], parts[di]
