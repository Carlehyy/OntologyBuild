...
class OpenJudgeEngine:
    ...
    async def _score(self, grader: Any, **data) -> tuple[float | None, str]:
        try:
            ...
            logger.warning("OpenJudge 评分器执行失败：%s", exc)
            return None, f"该维度评分执行失败：{exc}"
    ...
    async def evaluate(self, dim_keys: list[str], trace) -> dict[str, dict]:
        ...
        async def _run(key: str):
            if key in self._llm_graders:
                ...
                results[key] = {"raw": raw, "reason": reason}
            elif key == "trajectory":
                raw, reason = await self._score(
...
class FallbackEngine:
    ...
    async def evaluate(self, dim_keys: list[str], trace) -> dict[str, dict]:
        ...
        def _one(key: str) -> dict:
            if key == "action_loop":
                raw = detect_action_loop(trace.actions)
                reason = "重复动作比例检测（确定性算法）" if raw < 1.0 else ""
            ...
            if key == "trajectory":
                ...
                content = self._chat(_FALLBACK_PROMPTS[key],
                                     f"用户问题：{trace.query}\n\n完整轨迹：\n{transcript}")
            else:
                evidence = ""
                if trace.actions:
        ...
        tasks = [loop.run_in_executor(None, _one, k) for k in dim_keys]
