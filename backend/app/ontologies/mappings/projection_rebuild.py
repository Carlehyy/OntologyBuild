...
def rebuild_neo4j_projection(db: Session, ontology_id: str) -> bool:
    ...
    try:
        ...
        for entity in entities:
            ...
            business_properties = dict(entity.properties or {}).get(
                "__business_properties__",
            )
            if isinstance(business_properties, dict):
                # Reserved identity/display collisions remain private in SQL;
                # only their business view is flattened into the query graph.
                legacy_properties.update(business_properties)
            nodes[str(entity.id)] = {
                **legacy_properties,
                "id": str(entity.id),
