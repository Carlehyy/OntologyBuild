...
{{
  ...
  "field_mappings": [
    ...
    def _guess_type(col_name: str) -> str:
        ...
        用「词元」匹配而非裸子串匹配，避免误判：
        历史 bug 例如 ``"at" in "category"``（cATegory）→ datetime，
        ``"id" in "width"`` → integer，``"rate" in "generated"`` → float。

        规则：
        - 把列名按非字母数字（_、-、空格、驼峰）切成词元；
        - datetime/date：词元命中 或 以 _at/_date/_time 结尾（如 published_at）；
        - 其余类别按词元精确命中关键字集合。
        """
        import re as _re
        raw = col_name.lower()
        # 驼峰转下划线后再统一切词： publishedAt -> published_at
        snake = _re.sub(r"(?<=[a-z0-9])(?=[A-Z])", "_", col_name).lower()
        tokens = set(t for t in _re.split(r"[^a-z0-9]+", snake) if t)

        def has_token(*words: str) -> bool:
            return any(w in tokens for w in words)

        def ends_with(*sufs: str) -> bool:
            return any(snake.endswith(s) or raw.endswith(s) for s in sufs)

        # datetime：明确的时间词元，或常见时间后缀
        if has_token("date", "time", "datetime", "timestamp", "created", "updated",
                     "published", "modified") or ends_with("_at", "_date", "_time"):
            return "datetime"
        # integer：标识/计数类词元（注意 id 必须是独立词元，避免 width/video 误判）
        if has_token("id", "count", "num", "number", "age", "qty", "quantity", "index"):
            return "integer"
        # float：金额/评分/比率类词元
        if has_token("price", "amount", "score", "rate", "ratio", "pct", "percent",
                     "cost", "total"):
            return "float"
        # boolean：布尔类词元或 is_/has_ 前缀
        if has_token("flag", "active", "enabled", "disabled") \
                or snake.startswith(("is_", "has_")):
            return "boolean"
        return "string"
