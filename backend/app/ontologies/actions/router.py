...
def list_actions(ontology_id: str, db: Session = Depends(get_db), _=Depends(get_current_user)):
    # 单一可信数据源：正规本体 ActionType 存在时投影为 Action 形状
    from app.services.formal.legacy_bridge import has_formal_actions, list_actions_from_formal
    if has_formal_actions(db, ontology_id):
        return {"data": list_actions_from_formal(db, ontology_id)}
