...
def list_logic(ontology_id: str, db: Session = Depends(get_db), _=Depends(get_current_user)):
    # 单一可信数据源：正规本体函数存在时投影为 LogicRule 形状
    from app.services.formal.legacy_bridge import has_formal_functions, list_logic_from_formal
    if has_formal_functions(db, ontology_id):
        return {"data": list_logic_from_formal(db, ontology_id)}
