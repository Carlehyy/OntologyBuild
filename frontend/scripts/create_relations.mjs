...
for (const [name] of carriers) {
  const r = await api('POST', '/api/v1/ontologies/' + oid + '/entities', {
    name_cn: name, name_en: name, type: '承运商', description: `供应链承运商: ${name}`, confidence: 0.95, properties: { carrier_name: name }
  }, token);
...
for (const [name] of suppliers) {
  ...
  const r = await api('POST', '/api/v1/ontologies/' + oid + '/entities', {
    name_cn: name, name_en: name, type: '供应商', description: `供应链供应商: ${name}`, confidence: 0.95, properties: { supplier_code: name }
  }, token);
  const id = r.data?.id || r.id;
