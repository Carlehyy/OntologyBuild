 ...
 * 平台适配：中文默认文案与 24 小时制默认（hourCycle 可选 12）；其余逻辑保持上游原样。
 */
...
export const DEFAULT_WEEKDAY_LABELS: Record<DayKey, string> = {
  mon: "星期一",
  tue: "星期二",
  wed: "星期三",
  thu: "星期四",
...
export const WEEKDAYS: { key: DayKey; label: string }[] = (
  Object.keys(DEFAULT_WEEKDAY_LABELS) as DayKey[]
).map((key) => ({ key, label: DEFAULT_WEEKDAY_LABELS[key] }));

/** 组件可见文案默认值（中文）；经组件 `texts` prop 可整体或部分覆盖。 */
...
export const DEFAULT_TEXTS: SchedulerTexts = {
  unavailable: "不可用",
  addTime: "添加时间段",
  removeRange: "移除时间段",
  copyTimes: "复制到其他天",
...
/** 平台默认的 24 小时制显示（值本身就是 HH:mm）。 */
export const label24 = (v: string) => v;
...
export function buildOptions(
  ...
  for (let m = 0; m < 24 * 60; m += step) {
    ...
    out.push({
      value,
      label: hourCycle === 12 ? label12(value) : label24(value),
    });
  }
  return out;
}
