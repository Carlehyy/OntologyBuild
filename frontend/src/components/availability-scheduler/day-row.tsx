 ...
 * 平台适配：texts/days 从 index 下发，无障碍标签中文化；其余逻辑保持上游原样。
 */
...
export function DayRow({
  ...
  const actions = (
    <>
      <Tooltip content={texts.addTime}>
        <IconButton
          label={`${texts.addTime}：${label}`}
          reduce={reduce}
          onClick={addRange}
        >
          ...
          {state.enabled ? (
            state.ranges.map((r) => (
              ...
              >
                ...
                <Tooltip content={texts.removeRange}>
                  <IconButton
                    label={texts.removeRange}
                    reduce={reduce}
