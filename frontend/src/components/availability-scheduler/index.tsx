 ...
 * 平台适配：新增 weekdayLabels / hourCycle / texts 三个可覆盖项（中文为默认），
 * 星期标签与文案从 index 统一下发；其余逻辑保持上游原样。
 */
...
export interface AvailabilitySchedulerProps {
  ...
  step?: number;
  /** 时间显示制式；平台默认 24 小时制（"09:00"），可选 12（"9:00 AM"）。 */
  hourCycle?: 12 | 24;
  /** 星期标签覆盖；默认中文「星期一…星期日」。 */
  weekdayLabels?: Partial<Record<DayKey, string>>;
  /** 可见文案覆盖；默认中文（见 types.ts 的 DEFAULT_TEXTS）。 */
  texts?: Partial<SchedulerTexts>;
  className?: string;
}
...
export function AvailabilityScheduler({
  ...
  const groupId = useId();
  const options = useMemo(() => buildOptions(step, hourCycle), [step, hourCycle]);
  const idRef = useRef(0);
  const days = useMemo(
    () =>
      WEEKDAYS.map(({ key }) => ({
        key,
        label: weekdayLabels?.[key] ?? DEFAULT_WEEKDAY_LABELS[key],
      })),
    [weekdayLabels],
  );
  const t = useMemo(
