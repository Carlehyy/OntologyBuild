 ...
 * 平台适配：仅导入路径（popover-position 随 motion-ui 目录内聚）；
 * w-(--multi-select-trigger-width)（TW4 语法）→ w-[var(--multi-select-trigger-width)]（TW3 等效）。
 */
...
export function MultiSelectContent({
  ...
  return createPortal(
    <motion.div
      ...
      className={cn(
        "fixed z-[9999] w-[var(--multi-select-trigger-width)] overflow-hidden rounded-xl border border-border bg-background text-popover-foreground outline-none will-change-[height,transform]",
        className,
      )}
    >
      <motion.div
