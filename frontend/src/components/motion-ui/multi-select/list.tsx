 ...
 * 平台适配：仅导入路径（ease 随 motion-ui 目录内聚）；
 * scrollbar-none（TW4 工具类）→ [scrollbar-width:none]（TW3 等效，webkit 选择器保留）；
 * aria-label 与空态默认文案中文化。
 */
...
export function MultiSelectList({
  ...
  return (
    <div
      ...
      className={cn(
        "relative isolate max-h-64 overflow-y-auto overscroll-contain p-1.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden",
        className,
      )}
    >
      {children}
