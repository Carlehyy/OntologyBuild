 ...
 * 平台适配：hover:border-(--color-border-strong)（TW4 语法）→ hover:border-[var(--color-border-hover)]（TW3 等效）；
 * focus-within:ring-foreground/20 → focus-within:ring-ring（CSS 变量色不支持透明度修饰符，改用平台焦点环令牌）；
 * chip 移除钮 hover:bg-foreground/10 → hover:bg-background（同因，且与语义令牌约定一致）；
 * 占位与 aria 默认中文化。
 */
...
export function MultiSelectTrigger({
  ...
  return (
    <div
      ...
      className={cn(
        "relative z-20 flex min-h-11 w-full min-w-52 cursor-text items-center gap-2 rounded-xl border border-border bg-transparent px-2.5 py-1.5 text-sm text-foreground transition-[border-color] hover:border-[var(--color-border-hover)]",
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="-mr-1 grid size-5 shrink-0 place-items-center rounded-md text-muted-foreground outline-none transition-colors hover:bg-background hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
              >
