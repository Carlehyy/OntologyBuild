 ...
 * 平台适配：bg-background/10 → bg-black/25、bg-foreground/[0.05] → bg-muted（CSS 变量色
 * 不支持透明度修饰符的 TW3 改写）；关闭按钮与遮罩的 aria 文案中文化。
 */
          ...
          {({ isPresent, gate }) => (
            <>
              <motion.button
                ...
                className={cn(
                  "pointer-events-auto fixed inset-0 z-[100] h-full w-full cursor-default bg-black/25 backdrop-blur-sm",
                  backdropClassName,
                )}
              />
                  ...
                  >
                    ...
                    {showCloseButton ? (
                      <motion.button
                        ...
                        className="absolute right-4 top-4 inline-flex h-8 w-8 items-center justify-center rounded-full bg-muted text-muted-foreground transition-colors hover:bg-accent hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      >
