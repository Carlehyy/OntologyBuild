 ...
 *
 * - 用户名是账号唯一标识，只读展示，不允许修改；
 * - 邮箱可自助修改，成功后同步刷新本地登录态（auth-store）；
 * - 修改密码走既有 PUT /auth/password（需验证当前密码）；
 * - 用户私有环境变量：key/value 均为字符串，全量保存，服务端加密落库；
 *   本期仅做个人配置的保存与维护，不注入任何执行链路。
 */
  ...
  return (
    ...
    >
      ...
      <section aria-label="账号信息">
        <h4 className="text-sm font-medium text-[var(--color-text-primary)]">账号信息</h4>
      ...
      {/* 修改密码 */}
      <section aria-label="修改密码" className="mt-6 border-t border-[var(--color-border)] pt-5">
      ...
      {/* 私有环境变量 */}
      <section aria-label="私有环境变量" className="mt-6 border-t border-[var(--color-border)] pt-5">
        <h4 className="text-sm font-medium text-[var(--color-text-primary)]">私有环境变量</h4>
        <p className="mt-0.5 text-xs text-[var(--color-text-tertiary)]">
          ...
          {envLoading ? (
            ...
            <ul className="space-y-2">
              {envVars.map((item, index) => (
                <li key={index} className="grid grid-cols-[minmax(0,9rem)_minmax(0,1fr)_auto] items-center gap-2">
                  <input
                    value={item.key}
