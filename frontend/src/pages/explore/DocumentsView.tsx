...
import { FileText, Download, Loader2, ShieldAlert, Sparkles, Wand2 } from 'lucide-react'
import {
  explorationApi,
  type BxDocument,
  type BxDraft,
...
/** 需求文档视图：历史版本 + markdown 预览 + 生成本体模型（写入绑定草稿版本）。 */
export default function DocumentsView({
  sessionId, binding, onDraftCreated, onGenerate, documentGenerating, canGenerateDocument,
}: {
  sessionId: string
  /** 绑定态（会话锚定本体版本）：草稿固定写入绑定本体 */
  ...
  const generateDraft = async (force = false) => {
    ...
    try {
      const draft = await explorationApi.generateDraft(doc.id, {
        targetOntologyId: binding.ontologyId,
        force,
      })
      setGateBlock(null)
      setDraftRisk(null)
  ...
  return (
    <div className="flex h-full min-h-0" data-testid="requirements-view">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div data-testid="requirements-detail-header" className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-[var(--color-border)] px-5">
