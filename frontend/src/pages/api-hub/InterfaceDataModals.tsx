...
function SecretView({ secret, info, onDone }: { secret: string; info: ProxyInfo | null; onDone: () => void }) {
  return <div className="space-y-4"><div className="flex items-center gap-3 rounded-lg bg-emerald-50 p-4 text-emerald-800"><CheckCircle2 size={20} /><div><div className="text-sm font-semibold">密钥创建成功</div><div className="text-[11px]">完整密钥只显示这一次，请立即保存。</div></div></div><div className="flex gap-2"><input readOnly value={secret} className="h-10 min-w-0 flex-1 rounded-md border border-emerald-200 bg-emerald-50 px-3 font-mono text-xs" /><Button onClick={() => writeTextToClipboard(secret)}><Copy size={14} />复制密钥</Button></div><pre className="overflow-auto whitespace-pre-wrap rounded-md bg-slate-950 p-4 font-mono text-[11px] leading-5 text-slate-100">{`curl '${window.location.origin}${info?.path || '/proxy'}/<公开路径>' \\\n  -H '${info?.key_header || 'X-API-Hub-Key'}: ${secret}'`}</pre><div className="flex justify-end"><Button onClick={onDone}>我已保存</Button></div></div>
}
