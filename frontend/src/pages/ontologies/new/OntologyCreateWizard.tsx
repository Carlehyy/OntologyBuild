...
export default function OntologyCreateWizard() {
  ...
  if (step === 'select_mode') return (
    <div>
      <button onClick={() => navigate('/ontologies')} title="返回本体列表" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-black mb-6">
        <ArrowLeft size={16} />
      </button>
  ...
  if (step === 'fill_info') return (
    <div className="max-w-xl">
      <button onClick={() => setStep('select_mode')} title="返回选择方式" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-black mb-6">
        <ArrowLeft size={14} />
      </button>
