...
const PUBLISH_META: Record<string, { label: string; cls: string }> = {
  ...
  published: { label: '已发布', cls: 'bg-brand-soft text-brand-ink border-brand-line' },
}
...
export default function ManagedPipelinesPanel({ records, loading, expandedId, onExpand, onChanged, n8nApiUrl, onOpenWizard }: {
  ...
  return (
    <aside className="flex shrink-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-[0_8px_30px_rgba(15,23,42,0.04)] max-xl:max-h-[42%] max-xl:min-h-[180px] xl:min-w-0 xl:flex-1">
      <div className="flex items-center justify-between border-b border-border px-4 py-3.5">
        <div className="min-w-0">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-foreground shrink-0">
            <Workflow size={15} className="text-brand-ink" /> 可编排流水线
          </h2>
          <p className="mt-0.5 whitespace-nowrap text-[10px] text-[var(--color-text-tertiary)]">此工作区只展示处于未发布的n8n流水线</p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <button onClick={onChanged}
            className="flex items-center gap-1.5 rounded-lg border border-brand-line bg-brand-soft px-2.5 py-1.5 text-xs font-medium text-brand-ink transition-colors hover:border-brand-line hover:bg-brand-soft active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
      ...
      <div className="scrollbar-none flex-1 space-y-4 overflow-y-auto px-3 py-3">
        {loading ? (
          ...
          <div className="space-y-2 rounded-xl border border-dashed border-border bg-muted p-8 text-center text-[var(--color-text-tertiary)]">
...
function MiniGraph({ workflow }: { workflow: unknown }) {
  ...
  if (nodes.length === 0) return <div className="flex items-center justify-center h-full text-[11px] text-[var(--color-text-tertiary)]">暂无节点</div>
...
function RecordCard({ record: r, expanded, onToggle, n8nApiUrl, onOpenWizard }: {
  ...
  return (
    <div className={`overflow-hidden rounded-xl border transition ${expanded ? 'border-brand-line bg-brand-soft shadow-sm' : 'border-border bg-card hover:border-border hover:shadow-sm'}`}>
      <div className="flex items-start gap-2 px-3.5 py-3">
        <button onClick={onToggle} className="flex min-w-0 flex-1 items-start gap-2 text-left" aria-expanded={expanded}>
          {expanded ? <ChevronDown size={14} className="mt-0.5 shrink-0 text-[var(--color-text-tertiary)]" /> : <ChevronRight size={14} className="mt-0.5 shrink-0 text-[var(--color-text-tertiary)]" />}
          <div className="min-w-0 flex-1">
            ...
            {r.active && <span className="shrink-0 rounded border border-[color-mix(in_srgb,var(--color-success)_35%,transparent)] bg-[var(--color-success-bg)] px-1.5 py-px text-[10px] text-[var(--color-success)]">n8n 已激活</span>}
            </div>
            {r.description && <p className="mt-0.5 truncate text-[11px] text-muted-foreground">{r.description}</p>}
          </div>
        </button>
        <button onClick={() => { if (r.pipelineId) onOpenWizard(r.pipelineId) }}
          className="flex shrink-0 items-center gap-1 rounded-lg border border-brand-line px-2 py-1 text-xs text-brand-ink transition hover:bg-brand-soft"
          title="打开编辑向导">
          <Pencil size={11} /> 编辑
        </button>
        {canJump && (
          <button onClick={() => window.open(`${n8nWebUrl}/workflow/${r.n8nWorkflowId}`, '_blank')}
            className="flex shrink-0 items-center gap-1 rounded-lg border border-[color-mix(in_srgb,var(--color-info)_35%,transparent)] px-2 py-1 text-xs text-[var(--color-info)] transition-colors hover:bg-[var(--color-info-bg)]"
            title="跳转 n8n 工作流">
      ...
      {expanded && (
        <div className="space-y-2 border-t border-border px-3 py-2.5">
          {/* 节点连线图 */}
          <div className="h-[180px] w-full rounded-lg overflow-hidden border bg-muted">
            {detailLoading ? (
              <div className="flex items-center justify-center h-full text-[11px] text-[var(--color-text-tertiary)]">加载中…</div>
            ) : (
          ...
          {detail?.n8nError ? (
            <p className="text-[11px] text-[var(--color-warning)]">n8n 暂不可达：{detail.n8nError}</p>
          ) : null}
        </div>
      )}
    </div>
