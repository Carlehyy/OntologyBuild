...
| Neo4j/本体投影 | 移除 NetworkX、Formal/SQL 图读取兜底；新增 project durable fence、稳定 ID、业务字段无损别名和升级全量对账 | 图失败/半投影返回 503；空图仍是成功空结果；Raw Cypher 仅接受每个图变量均显式按本体隔离的单段只读查询 |
| MinIO/文件资产 | 新对象只写环境 MinIO；回归时代数据库托管端点仅供存量读取/删除/列举迁移 | 历史 `local://` 仅保留只读/迁移 |
| n8n/Chromium CDP | n8n 纳入阻塞型启动依赖；CDP 地址配置必需且纳入深度 readiness，但连通失败不终止 API | CDP 仅接受服务根地址并统一探测 `/json/version`；n8n PUT 收紧为 409，设置页改只读 |
...
Alembic `0056_ontology_projection_fence` 会把升级前的本体标记为
...
- PostgreSQL→Neo4j 投影不会用图 envelope 覆盖同名业务 `id`/`updated_at`；Raw
  Cypher 无法通过第二段 MATCH、子查询、未限定关系或弱化条件跨本体读取；
...
|---|---|---|---|
| 文档 | `node scripts/ci/check-markdown-links.mjs`；`bash scripts/ci/check-repository-hygiene.sh`；`git diff --check` | 最终合并分支 96 个 Markdown/510 个链接零错误零警告；仓库卫生通过；diff check 通过 | 本地工作树 |
| 后端定向 | 图/Mapping/正式模型、基础设施/部署/健康/迁移、架构边界、投影冲突回归、旧迁移脚本与性能相关 pytest | 源提交分组分别为 108、99、30、29、10 passed；与旧抽取退役提交合并后，迁移/架构/健康组合 71 passed；最终审计修复后 graph 78 passed、mapping 122 passed、Raw Cypher 定向 29 passed、性能 9 passed；分组之间存在重叠，不合计为总用例数 | 本地工作树 |
| 配置中心 | `cd config && uv run pytest -q` | 42 passed | 本地工作树 |
| 后端完整 | `cd backend && uv run pytest -q --disable-warnings --ignore tests/v2/perf` | 最终合并分支 1825 passed，1 skipped | 本地工作树 |
| 前端静态/mocked | unit、feature boundary、classification、lint、build、mocked E2E | unit 12 passed；最终合并分支 classification 49 个 spec 唯一归类；boundary、lint、build 通过；mocked E2E 81 passed | 本地工作树 |
| 隔离真实依赖栈 | 临时 PostgreSQL 16、Redis、Neo4j、MinIO、n8n、Chromium CDP 与真实 Celery worker | 最终合并链的新库→0056、存量 0054→0055 旧抽取退役→0056 投影围栏，以及 0056→0054→0056 降级重升均通过；全部真实探针、Celery PONG、MinIO CRUD、SQL→Neo4j 稳定 ID/业务字段/嵌套 JSON 对账和完整 lifespan ready=200 通过；审计修复后另以真实 Neo4j 5.26 验证 `id`/`updated_at`、二阶别名冲突完整快照及稳定关系读写 | 本地隔离环境；短期 n8n Key 已吊销，临时目录和全部容器已清理 |
...
| 部署/回滚 | 部署脚本语法、workflow YAML、配置权威/白名单守卫；生产 canary 与回滚 | 脚本/workflow 语法和部署守卫通过；真实 PostgreSQL 迁移 step 固定使用 test 配置边界；未执行生产 canary、真实生产部署或生产回滚演练 | 发布前由运维在受控环境补充 artifact |
