# 归档：nano-ontoprompt README

> 这是项目更名前的历史说明，链接、版本和功能描述可能已经过期。
> 当前项目入口以仓库根目录 `README.md` 和 `docs/index.md` 为准。
...
深入理解 Ontology-as-a-Service 架构设计 — 包括 Object/Link/Function/Governance 设计模式、多租户隔离、临床筛查工作流、生产部署 Checklist — 详见 **[Ontology 架构参考](../reference/ontology.md)**（2727 行）。
...
`/uploads/object-storage`），当前部署细节见 [部署说明](../operations/deployment.md)。
...
实时画面默认优先使用 WebSocket；若终端安全策略或企业代理阻止 WebSocket 握手、连接在首帧前超时，
或连接中途断开，页面会自动切换为鉴权 HTTPS 截图轮询与 HTTPS 输入指令，无需用户手动选择。兼容模式
继续保留投影、鼠标、键盘、滚轮和人工接管语义，并用可续期短租约在页面异常退出后自动释放接管。
`STEWARD_BROWSER_HTTP_LEASE_SECONDS` 可调整租约时长，
`STEWARD_BROWSER_HTTP_FRAME_INTERVAL_MS` 可调整兼容模式帧间隔。
该降级针对“实时浏览器”弹窗与平台之间的画面/控制通道；下述“我的电脑”助手承载的是远程 CDP
隧道，目前仍要求本机能够主动建立 WSS 连接。
